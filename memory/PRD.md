...
- v9: (1) Pre-trip hype reminder emails — _reminder_loop() hourly asyncio task (startup) auto-sends when 0<=days_until<=3 and reminder_sent!=True; POST /api/trips/{id}/send-reminder manual trigger (+ 'Send pre-trip hype email' button in TripDetailPage invite box); recipients = owner + linked members + accepted invite emails; content: gpt-5.4 generated hype_line + 10 place-specific packing checklist items (DEFAULT_CHECKLIST fallback); crazy email template _reminder_email_html (T-MINUS header, checklist ☐ rows in dashed ticket, CTA to {trip.origin_url}/planner/{id}; trips now store origin_url from wizard). (2) Read receipts — POST /api/rooms/{id}/read sets reads.{user_id}=iso, GET /api/rooms/{id}/reads; SquadChatPage: markRead on open + on new msgs, reads fetched each 2.5s poll, my messages show ✓ (sent) → ✓✓ (seen when ALL other members' read ts >= msg ts). Verified live: tick flip confirmed in browser. 17/17 tests pass.

- v10 (bugfix): 'invite emails not going' — preview SMTP was fine; hardened: _send_email_sync now multipart plain+HTML with Date/Message-ID/Reply-To (spam-score), logs '[EMAIL sent via gmail]', and RAISES when EMAIL_PROVIDER=gmail but creds missing (no more silent fake success — likely the production symptom since prod was deployed before Gmail creds existed; user must REDEPLOY). Invite response sent[] items now {email, link}; failed[] adds reason; GET /api/trips/{id}/invites includes token (owner-only) → UI copy-invite-link button per pending row + auto-copies link on send failure. 9/9 verified by testing agent.

- v11 (bugfix round 2): mic + invite-mail re-reports (user testing outdated PRODUCTION). Preview re-verified by testing agents: mic 3/3 scenarios (widget + squad voice note + permission-denied UX), invites 7/7 incl. sending to a DIFFERENT user + magic-link accept. Hardened: iframe-aware mic error toast with 'Open in new tab' action (preview frames block getUserMedia); invite email now shows plain fallback link under CTA. Deployment readiness: N+1 fixed in GET /api/trips (batched $in), .gitignore .env-blocking patterns removed — deployment_agent PASS. USER MUST REDEPLOY to fix production.

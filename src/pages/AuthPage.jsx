...
export default function AuthPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Note: Ready for Firebase linking — current bypass for zero-network demo
      if (mode === 'login') {
    ...
    <div className="min-h-screen bg-[#030303] flex items-center justify-center p-6 md:p-10 relative overflow-hidden">
      ...
      <div className="w-full max-w-xl relative z-10">
        ...
        <header className="text-center mb-16">
          <h1 className="text-[10vw] md:text-[8vw] font-[900] leading-none mb-4 uppercase font-bebas text-white">
        ...
        <div className="silicon-glass">
            ...
            {['login', 'signup'].map(m => (
              <button
                ...
                className={`flex-1 py-5 rounded-xl font-black text-sm uppercase tracking-widest transition-all ${
                  mode === m ? 'bg-orange-500 text-white shadow-[0_0_20px_rgba(255,77,0,0.3)]' : 'text-white/30 hover:text-white'
                }`}
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <button type="submit" disabled={loading} className="w-full btn-launch py-10 text-3xl rounded-[2rem] disabled:opacity-50">
              {loading ? (
                <span className="flex items-center justify-center gap-4"><span className="w-8 h-8 border-4 border-white/20 border-t-white rounded-full animate-spin" />AUTHENTICATING...</span>
              ) : (
                <>{mode === 'login' ? 'ENTER HQ' : 'JOIN SQUAD'} <ArrowRight size={40} weight="bold" /></>
              )}
            </button>
          </form>
          <div className="mt-12 text-center">
            <button onClick={() => { login('demo@travelo.app', 'demo'); navigate('/dashboard'); }} className="text-white/20 font-bold text-sm uppercase tracking-widest hover:text-orange-500 transition-colors italic underline underline-offset-4">
